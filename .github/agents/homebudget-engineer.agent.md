...
- `src/components/success-sound.ts` (native no-op) / `success-sound.web.ts` (real implementation, resolved automatically by the bundler per platform) exports `playSuccessSound()` — a short two-note chime synthesized with the Web Audio API (no audio asset needed). Wired into add-transaction, add joint payment, add extra income, and marking a goal complete. Web-only by design since the app has no native build; extend to more screens by importing from `@/components/success-sound`.
...
- Savings includes manual investment-account tracking in `investment-accounts-panel.tsx`: accounts can be added, edited, deleted, and assigned `TFSA`, `RRSP`, `DCPP`, or `OTHER`; dated balance snapshots can be added/deleted; and `investment-trend-chart.tsx` plots summed TFSA versus summed RRSP/DCPP balances over time. This is separate from cash-flow savings totals because market-value snapshots are not transaction cash flow. `ensureFeatureSchema()` seeds three starter accounts (`Wealthsimple TFSA`, `Quadrus TFSA`, `Quadrus RRSP`) only the first time `investment_accounts` is created (checked via `to_regclass`/`OBJECT_ID` before the `CREATE TABLE IF NOT EXISTS`), not on every server start — a per-row `ON CONFLICT`/`IF NOT EXISTS (name)` check alone would silently re-add them after a user deletes them, since restarting the API service re-runs schema bootstrap.
- `dashboard-screen.tsx` shows a monthly reminder (via the shared `useConfirm()` dialog) once per calendar month on load, prompting the user to update Savings-tab investment balances; either button (Go to Savings / Not now) marks the month as shown in `localStorage` (`homebudget-savings-reminder-shown`, web-only, per-browser — not synced between Peter's and Sailah's devices).
...
- An hourly maintenance check runs via `src/server/health-check.service` + `health-check.timer` using `scripts/health-check.sh`: verifies core services (`budget-api.service`, `expo-app.service`, `nginx`, `wg-quick@wg0`, `dnsmasq`, `postgresql`) are active, the API/web respond locally, the Pi's public IP matches what `sphomebudget.duckdns.org` resolves to (a DuckDNS/cron staleness mismatch here silently broke the VPN for weeks in Sept 2026 — see repo memory for the full incident), root disk usage stays under 90%, and the last successful backup (`~/.local/share/homebudget/backup-status.json`) is under 40 days old. Results append to `/home/pstrzelbicki/health-check.log` (log-only, no push/email alerting — checked manually over SSH). `deploy-pi.ps1`/`deploy-pi-remote.sh` strip CRLF from every `scripts/*.sh` file on the Pi before running anything, since Windows-authored scripts can otherwise fail with a trailing `$'\r': command not found`.
- Additional stability hardening deployed alongside the health check: `src/server/journald-homebudget.conf` caps the systemd journal (300M/90 days) so logs can't fill the SD card, and `src/server/service-restart-override.conf` gives `nginx`/`dnsmasq` the same `Restart=on-failure` drop-in that `budget-api.service`/`expo-app.service` already had (they defaulted to `Restart=no`). `wg-quick@wg0` is a one-shot unit with no persistent process, so it relies on the health check rather than a restart policy.
