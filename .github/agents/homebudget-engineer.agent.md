...
- The Budget screen (`budget-screen.tsx`) allows navigating to any month, including future months, for preparation. Future-month budget lines, caps, and templates remain editable but are excluded from current-period graphs, stats, summaries, and insights. It supports a per-month `total_budget` cap with a warning only when the cent-rounded allocation is genuinely above the cap, plus named reusable budget templates that can be saved, applied, or deleted.
